// DES (images).cpp : This file contains the 'main' function. Program execution begins and ends there.
//
...
void binary_to_char(int binary[], uchar charac[]) {
    ...
    //        dec[j] += binary[i] * int(pow(2, (sizeof(binary) - 1 - i)));
    //    }

    //    charac[j] = (uchar)dec[j];
    //}
...
int main()
{
    ...
    cout << "Input your image name and path, or press d for default." << endl;
    string imname;
    cin >> imname;
    if (imname == "d") {
        imname = "bmp_24.bmp";
    ...
    cout << "Input your key in hexadecimal, or press d for default. Key must be exactly 16 places long." << endl;
    string key;
    cin >> key;
    ...
    // to hold cipher image channels
    //uchar cipher_char[3][400];
    //uchar** cipher_char = new uchar* [3];
    //for (int i = 0; i < 3; i++) {
    //    cipher_char[i] = new uchar[40000];
    //}
    vector<vector<uchar>> cipher_char(num_channels, vector<uchar>(ttl_pix));
    //for (int i = 0; i < 3; i++) {
    //    for (int j = 0; j < ttl_pix; j++) {
    //        cipher_char[i][j] = 0x20;
    //    }
    //}
    // to hold decoded image channels
    //uchar decode_char[3][4000];
    //uchar** decode_char = new uchar* [3];
    //for (int i = 0; i < 3; i++) {
    //    decode_char[i] = new uchar[ttl_pix];
    //}
    vector<vector<uchar>> decode_char(num_channels, vector<uchar>(ttl_pix));

    int num_blocks = ceil(ttl_pix / 8.0);
    //uchar input_arr[5000][8] = { "" };
    vector<vector<uchar>> input_arr(num_blocks, vector<uchar>(8));
    //uchar** input_arr = new uchar * [num_blocks];
    //for (int i = 0; i < num_blocks; i++) {
    //    input_arr[i] = new uchar[8];
    //}

    int mode;
    cout << "Type 1 for ECB or 2 for CBC" << endl;
    cin >> mode;
    ...
    //cout << "cipher channels array[2] is ";
    //cout << cipher_channels_arr[2] << endl;

    //cout << "cipher char [2] is ";
    //for (int i = 0; i < ttl_pix; i++) {
    //    cout << +cipher_char[2][i] << " ";
    //    if (i % 20 == 0) {
    //        cout << endl;
    //    }
    //}
    ...
    //for (int i = 0; i < 3; i++) {
    //    delete[] cipher_char[i];
    //}
    //delete[] cipher_char;
    //for (int i = 0; i < 3; i++) {
    //    delete[] decode_char[i];
    //}
    //delete[] decode_char;
    //for (int i = 0; i < num_blocks; i++) {
    //    delete[] input_arr[i];
